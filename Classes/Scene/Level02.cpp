...
Scene* Level02::createScene()
{
	...
	auto pauseMenuItem = MenuItemImage::create(
		"Buttons/buttonPause.png", "Buttons/buttonPauseClicked.png", CC_CALLBACK_1(Level02::menuPauseCallback, layer));
...
bool Level02::init()
{
	...
	smallMap = Sprite::create("Props/smallMap3.png");
	...
	TMXObjectGroup* groupShop = map->getObjectGroup("obj");
	ValueMap spawnhopPoint = groupShop->getObject("shop");
	float xShop = 2*spawnhopPoint["x"].asFloat();
	float yShop = 2*spawnhopPoint["y"].asFloat();
...
/////////////////////////宝箱的设置
	...
	// ////////////////////////////////////////////////////////////////////
	this->schedule(schedule_selector(Level02::enemyAttackUpdate), 1.0f, kRepeatForever, 0);//敌人的攻击
	this->schedule(schedule_selector(Level02::enemyMoveUpdate), 2, -1, 0);//敌人的移动
	this->schedule(schedule_selector(Level02::armorRecoverUpdate), 2.0f, kRepeatForever, 0);//护甲的恢复
	this->schedule(schedule_selector(Level02::heroDieUpdate), 3.0f, kRepeatForever, 2.0f);
	this->schedule(schedule_selector(Level02::groundAttackUpdate), 0.5f, kRepeatForever, 0);
...
void Level02::createMonster(char arr[5][4], const std::string& location, int num)
{
	...
	for (int i = 0; i < 5; i++)
	{
		...
		Creep* monsterTemp;
		if (num / 5 == 0)
			monsterTemp = Creep::create(location, NORMAL, ENEMY_REMOTE2, CREEP_HP_3, MONSTER_ATTACK_LOW, true);
		else if (num / 5 == 1)
			monsterTemp = Creep::create(location, NORMAL, ENEMY_REMOTE2, CREEP_HP_2, MONSTER_ATTACK_LOW, true);
		else if (num / 5 == 2)
			monsterTemp = Creep::create(location, NORMAL, ENEMY_REMOTE, CREEP_HP_2, MONSTER_ATTACK_LOW, true);
		else if (num / 5 == 3)
			monsterTemp = Creep::create(location, NORMAL, ENEMY_REMOTE2, CREEP_HP_2, MONSTER_ATTACK_LOW, true);
		else if (num / 5 == 4)
			monsterTemp = Creep::create(location, NORMAL, ENEMY_REMOTE, CREEP_HP_3, MONSTER_ATTACK_LOW, false);
		else if (num / 5 == 5)
			monsterTemp = Creep::create(location, NORMAL, ENEMY_REMOTE, CREEP_HP_3, MONSTER_ATTACK_LOW, false);
...
void Level02::attackMelee(Hero* hero)//没有攻击目标时的随机攻击
{
...
bool Level02::attackTowardsTarget(Hero* hero)
{
...
void Level02::attackRemotely(Vec2 enemy, Hero* hero)
{
	int tileGid;
	float relativeX, relativeY, relativeR, searchX, searchY;
	float cosAlpha, sinAlpha;//角度
...
void Level02::setPlayerPosition(Point position, Point anotherPos)
{
	//从像素点坐标转化为瓦片坐标
...
void Level02::nextScene()
{
	auto scene = Level03::createScene();
	////预处理
	scene->removeChildByTag(TAG_OF_LEVEL_03);
	//
	auto stage = Level03::create();
...
void Level02::initProps()
{
	...
	////第二个宝箱
	weaponVec.at(2)->setPosition(boxPositionVec[1]);
	insidePropsVec.pushBack(weaponVec.at(2));
	this->addChild(weaponVec.at(2), 2);
	this->addChild(bulletVec.at(2), 2);
	weaponVec.at(2)->setVisible(false);
	bulletVec.at(2)->setVisible(false);

	//
	////第三个宝箱
	auto drugMP02 = Props::create("Props/healMagic.png", PROPS_DRUG_MP, boxPositionVec[2].x, boxPositionVec[2].y, 50);
	drugMP02->setPosition(boxPositionVec[2].x + 50, boxPositionVec[2].y);
	...
	auto gold02 = Props::create("Props/gold.png", PROPS_GOLD, boxPositionVec[2].x, boxPositionVec[2].y, 50);
	gold02->setPosition(boxPositionVec[2]);
	...
	////第四个宝箱
	auto drugHP = Props::create("Props/healBlood.png", PROPS_DRUG_HP, boxPositionVec[3].x, boxPositionVec[3].y, 50);
	drugHP->setPosition(boxPositionVec[3]);
	...
	////第五个宝箱--武器宝箱（随机武器）
	weaponVec.at(4)->setPosition(boxPositionVec[4]);
	insidePropsVec.pushBack(weaponVec.at(4));
	this->addChild(weaponVec.at(4), 2);
	this->addChild(bulletVec.at(4), 2);
	weaponVec.at(4)->setVisible(false);
	bulletVec.at(4)->setVisible(false);
	
}

void Level02::initShop(float x, float y)
{
	//商店的初始化
	auto shop = Shops::create("Shops/shopWithTable.png", x, y);
	shop->setPosition(Vec2(shop->getX(), shop->getY()));
	addChild(shop, 0);

	//血瓶
	auto drugHP = Props::create("Props/healBlood.png", PROPS_DRUG_HP, shop->getLocation1X(), shop->getLocationY(), 50);
	drugHP->setPosition(shop->getLocation1X(), shop->getLocationY());
	addChild(drugHP);
	propsOnSaleVec.pushBack(drugHP);

	//蓝瓶
	auto drugMP = Props::create("Props/healMagic.png", PROPS_DRUG_MP, shop->getLocation2X(), shop->getLocationY(), 50);
	drugMP->setPosition(shop->getLocation2X(), shop->getLocationY());
	addChild(drugMP);
	propsOnSaleVec.pushBack(drugMP);

	//紫瓶
	auto drugHPMP = Props::create("Props/drugHPMP.png", PROPS_DRUG_HPMP, shop->getLocation3X(), shop->getLocationY(), 50);
	drugHPMP->setPosition(shop->getLocation3X(), shop->getLocationY());
	addChild(drugHPMP);
	propsOnSaleVec.pushBack(drugHPMP);
}

void Level02::buyProps()
{
	if (!this->getKnight()->getAlreadyDead())
	{
		Vec2 position = this->getKnight()->getPosition();
		for (int i = 0; i < propsOnSaleVec.size(); i++)
		{
			...
			if ((props->isInRange(position)) && (gold >= price))
			{
				props->setVisible(false);
				props->makeEffect(this, this->getKnight());
...
void Level02::groundAttackUpdate(float delta)
{
		...
		if (groundAttack == "true")
		{
			...
			if (knight->getArmor() < 0)
			{
				...
				if (knight->getHP() <= 0)
				{
					...
					knight->initWithFile("Heroes/knight1.1Dead.png");
					knight->stopAllActions();
				}
				else
					knight->setAlreadyDead(false);
			}
			this->schedule(schedule_selector(Level02::returnColor), 0, 1, 0.1f);
...
void Level02::returnColor(float dt)
{
	auto knight = this->getKnight();
	knight->setColor(colorKnight);
}

void Level02::initWeapon()
{
